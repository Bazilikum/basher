...
function determineProjectPath() {
    ...
    logger.warn('No project directory specified. Using home directory as fallback. ' +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === 'execute_command') {
            ...
            const { command, cwd, stdin, timeout, background, title, waitFor, waitTimeout, parseAs, outputMode, trackProgress: doTrackProgress, retry, diffWithLast, analyzeFailure: doAnalyzeFailure, timestamps } = params;
            // ========================================================================
            // WHITELIST CHECK - Block non-whitelisted commands
            // ========================================================================
            const whitelistCheck = whitelistManager.check(command);
            if (!whitelistCheck.allowed) {
                logger.warn({ command, commandBase: whitelistCheck.commandBase }, 'Command blocked: not whitelisted');
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
        ...
        if (name === 'get_command_by_process_id') {
            ...
            if (isStale) {
                logger.warn({ processId, foundCommandId: command.id, foundTimestamp: command.timestamp }, 'Found potentially stale command - process ID may have been reused');
            }
            return {
                content: [
                    {
        ...
        if (name === 'advanced_search') {
            ...
            const results = historyManager.advanced.advancedSearch(params.query, params.limit, params.outputLevel, params.contextLines, params.filters);
            const data = {
                query: params.query,
        ...
        if (name === 'compare_executions') {
            ...
            const diff = historyManager.advanced.compareExecutions(params.commandId1, params.commandId2);
            if (!diff) {
                return {
                    content: [
                        {
        ...
        if (name === 'get_last_failures') {
            ...
            const results = historyManager.advanced.getLastFailures(params.limit, params.outputLevel, params.since);
            return {
                content: [
                    {
                        type: 'text',
        ...
        if (name === 'get_similar_commands') {
            ...
            const results = historyManager.advanced.getSimilarCommands(params.commandId, params.limit, params.outputLevel);
            return {
                content: [
                    {
                        type: 'text',
        ...
        if (name === 'get_command_chain') {
            ...
            const results = historyManager.advanced.getCommandChain(params.startId, params.maxCommands, params.outputLevel);
            return {
                content: [
                    {
                        type: 'text',
