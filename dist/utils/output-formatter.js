 ...
 */
export function formatCommandOutput(cmd, outputLevel = 'full', contextLines = 3, searchQuery) {
    switch (outputLevel) {
        case 'summary':
 ...
 */
export function formatSummary(cmd) {
    return {
        id: cmd.id,
        command: cmd.command,
        cwd: cmd.cwd,
 ...
 */
export function formatPreview(cmd, previewLines = 5) {
 ...
 */
export function formatExcerpts(cmd, searchQuery, contextLines = 3) {
    const summary = formatSummary(cmd);
    const excerpts = [];
    if (!searchQuery) {
 ...
 */
function findMatchingLinesWithContext(lines, regex, contextLines) {
    const results = [];
 ...
 */
export function deduplicateLines(text, showCount = true) {
 ...
 */
export function compressOutput(text, maxLines = 100, keepFirst = 50, keepLast = 50) {
    const lines = text.split('\n');
