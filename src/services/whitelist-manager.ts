...
export class WhitelistManager {
   ...
   */
  private initializeWithDir(basherDir: string): void {
  ...
  add(
    ...
    const validatePatterns = (patterns: string[] | undefined, name: string): string | null => {
    ...
    logger.info(
      { commandBase: normalized, description: options?.description, allowedArgs: options?.allowedArgs, blockedArgs: options?.blockedArgs },
      `${action} command in whitelist`
    );
